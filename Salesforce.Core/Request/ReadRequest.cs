...
namespace Salesforce
{
	public class ReadRequest : IAuthenticatedRequest
	{
		...
		public OAuth2Request ToOAuth2Request (ISalesforceUser user)
		{
			...
			var oauthRequest = new OAuth2Request (Method, uri, Resource.Options.Where (kvp => kvp.Value.JsonType == JsonType.String).ToDictionary (k => k.Key, v => (string) v.Value),Headers, user);
			return oauthRequest;
		}
